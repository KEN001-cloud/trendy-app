...
,{id: 9,value: "October"},{id: 10,value: "November"},{id: 11,value: "December"}]
  ...
  const [selectedMonth, setSelectedMonth] = useState(months.find((m) => m.id === today.getMonth()).value);
  ...
  const handleSelect = (id,e) => {
    setAnchorEl(null);
    setClickedMonth(id);
    setSelectedMonth(months.find((obj) => obj.id === id).value);
  };
  ...
  const name = "MonthlyReport_" + today.getDay() + "-" + today.getMonth() + "-" + today.getFullYear() + ".pdf";
  ...
  return (
    ...
    <Button style={{margin: "10px"}} aria-controls="simple-menu" aria-haspopup="true" onClick={handleClick}>
    {selectedMonth}
  </Button>
    ...
    >{months.map((month) => {
      return(
        <MenuItem onClick={(e) => handleSelect(month.id,e)}>{month.value}</MenuItem>
      )
    })}
    </Menu>
